...
pipeline {
    ...
    stages {
        ...
        stage('commit version update') {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'gitlab-access', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                        ...
                        sh 'git branch'
                        sh 'git config --list'
                        sh "git remote set-url origin https://github.com/ManideepM777/java-maven-app.git"
                        sh 'git add .'
